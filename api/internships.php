...
function handleUpload($file) {
    ...
    $filename = 'internship_' . uniqid() . '.' . $ext;
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    if (isset($_GET['id'])) {
        ...
        $stmt = $conn->query("SELECT * FROM internships ORDER BY created_at DESC");
...
if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
    ...
    if (!empty($input['id'])) {
        ...
        $stmt = $conn->prepare("UPDATE internships SET company_name=?, description=?, image_url=? WHERE id=?");
        $stmt->execute([$input['company_name'], $input['description'], $image_url, $input['id']]);
    } else {
        $stmt = $conn->prepare("INSERT INTO internships (company_name, description, image_url) VALUES (?, ?, ?)");
        $stmt->execute([$input['company_name'], $input['description'], $image_url]);
    }
    echo json_encode(['success' => true]);
    exit;
}
